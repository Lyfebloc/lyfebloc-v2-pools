...
export default class LinearPool extends BasePool {
  ...
  private _lbptToken: ILinearPoolToken;
  ...
  get tokens() {
    return [this._mainToken, this._wrappedToken, this._lbptToken];
  }
  ...
  public swapGivenIn(
    ...
    if (tokenIn.symbol === this._lbptToken.symbol) {
      if (tokenOut.symbol === this._mainToken.symbol) {
        scaledAmountOut = math._calcMainOutPerLbptIn(
          ...
          // MAX_TOKEN_BALANCE is always greater than LBPT balance
          this.MAX_TOKEN_BALANCE.minus(this._lbptToken.balance),
          {
        ...
        scaledAmountOut = math._calcWrappedOutPerLbptIn(
          this._upScale(amountIn, tokenIn.decimals),
          this._upScale(this._mainToken.balance, this._mainToken.decimals),
          this._upScale(
            this._wrappedToken.balance,
      ...
      } else if (tokenOut.symbol === this._lbptToken.symbol) {
  ...
  public swapGivenOut(
    ...
    if (tokenOut.symbol === this._lbptToken.symbol) {
      ...
      } else if (tokenIn.symbol === this._lbptToken.symbol) {
        scaledAmountIn = math._calcLbptInPerMainOut(
          this._upScale(amountOut, tokenOut.decimals),
          this._upScale(this._mainToken.balance, this._mainToken.decimals),
          this._upScale(
            this._wrappedToken.balance,
      ...
      if (tokenIn.symbol === this._mainToken.symbol) {
        ...
        scaledAmountIn = math._calcLbptInPerWrappedOut(
          this._upScale(amountOut, tokenOut.decimals),
          this._upScale(this._mainToken.balance, this._mainToken.decimals),
          this._upScale(
            this._wrappedToken.balance,
