...
export const _calcBptOutGivenExactTokensIn = (
  ...
  if (invariantRatio.gte(fp.ONE)) {
    return fp.mulDown(lbptTotalSupply, fp.sub(invariantRatio, fp.ONE));
  } else {
    return fp.ZERO;
  }
};
...
export const _calcTokenInGivenExactBptOut = (
  ...
  *****************************************************************************************/
  ...
  const invariantRatio = fp.divUp(
    fp.add(lbptTotalSupply, bptAmountOut),
    lbptTotalSupply
  );
...
export const _calcLbptInGivenExactTokensOut = (
  balances: BigNumber[],
  normalizedWeights: BigNumber[],
  ...
  return fp.mulUp(lbptTotalSupply, fp.complement(invariantRatio));
};

export const _calcTokenOutGivenExactLbptIn = (
  balance: BigNumber,
  normalizedWeight: BigNumber,
  lbptAmountIn: BigNumber,
  lbptTotalSupply: BigNumber,
  swapFee: BigNumber
): BigNumber => {
  /*****************************************************************************************
  ...
  // a = amountOut                                                                        //
  // b = balance                     /      /    lbpt - llbptIn    \    (1 / w)  \           //
  // llbptIn = lbptAmountIn    a = b * |  1 - | ------------------- | ^           |          //
  // lbpt = lbptTotalSupply            \      \        lbpt        /             /           //
  // w = weight                                                                           //
  *****************************************************************************************/
  ...
  // Because (lbpt - llbptIn) / lbpt <= 1, the exponent rounds down

  // Calculate the factor by which the invariant will decrease after burning `lbptAmountIn`
  const invariantRatio = fp.divUp(
    fp.sub(lbptTotalSupply, lbptAmountIn),
    lbptTotalSupply
  );
...
export const _calcTokensOutGivenExactLbptIn = (
  balances: BigNumber[],
  lbptAmountIn: BigNumber,
  lbptTotalSupply: BigNumber
): BigNumber[] => {
  ...
  // (formula per token)                                                                  //
  // ao = amountOut                  /   llbptIn   \                                        //
  // b = balance           ao = b * | ----------- |                                       //
  // llbptIn = lbptAmountIn             \    lbpt    /                                        //
  ...
  *****************************************************************************************/
  ...
  const lbptRatio = fp.divDown(lbptAmountIn, lbptTotalSupply);

  const amountsOut = new Array<BigNumber>(balances.length);
  for (let i = 0; i < balances.length; i++) {
    amountsOut[i] = fp.mulDown(balances[i], lbptRatio);
  }
...
// Convenience method needed by the AUTO package (adapted from _calcBptOutGivenExactTokensIn)
...
// Convenience method needed by the AUTO package (adapted from _calcLbptInGivenExactTokensOut)
export function _calcLbptInGivenExactTokenOut(
  balance: BigNumber,
  normalizedWeight: BigNumber,
  amountOut: BigNumber,
  lbptTotalSupply: BigNumber,
