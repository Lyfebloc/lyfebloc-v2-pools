import { getLyfeblocContractAddress } from "@lyfebloc/v2-deployments";
...
export const initFromOnchain = async (
  ...
  const vaultAddress = await getLyfeblocContractAddress(
    "20210418-vault",
    "Vault",
    network
  );
  ...
  const lbptTotalSupply = formatEther(await pool.totalSupply());
...
export const initFromSubgraph = async (
  ...
  const lbptTotalSupply = pool.totalShares;
